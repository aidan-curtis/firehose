...
class Cell2FireC:
    ...
    def run(self):
        ...
        execArray = [
            ...
            self.args.InFolder,
            "--output-folder",
            self.args.OutFolder if (self.args.OutFolder is not None) else "",
            "--ignitions" if (self.args.ignitions) else "",
            "--sim-years",
            str(self.args.sim_years),
            "--nsims",
            str(self.args.nsims),
            "--grids" if (self.args.grids) else "",
            "--final-grid" if (self.args.finalGrid) else "",
            "--Fire-Period-Length",
            str(self.args.input_PeriodLen),
            "--output-messages" if (self.args.OutMessages) else "",
            "--weather",
            self.args.WeatherOpt,
            "--nweathers",
            str(self.args.nweathers),
            "--ROS-CV",
            str(self.args.ROS_CV),
            "--IgnitionRad",
            str(self.args.IgRadius),
            "--seed",
            str(int(self.args.seed)),
            "--nthreads",
            str(int(self.args.nthreads)),
            "--ROS-Threshold",
            str(self.args.ROS_Threshold),
            "--HFI-Threshold",
            str(self.args.HFI_Threshold),
            "--bbo" if (self.args.BBO) else "",
            "--HarvestPlan",
            self.args.HCells if (self.args.HCells is not None) else "",
    ...
    def getData(self):
        ...
        (
            ...
            CoordCells,
            CellSide,
        ) = ReadDataPrometheus.ForestGrid(ForestFile, FBPDict)
        ...
        FTypeCells, StatusCells, RealCells, Colors = InitCells(
            self._NCells, self._FTypes2, ColorsDict, GForestType, GForestN
        )
    ...
    def DummyMsg(self):
        ...
        existingIDs = [
            int(re.match("MessagesFile(\d+)", MessagesFiles[i]).groups()[0])
            for i in range(0, len(MessagesFiles))
        ]
        ...
        for i in range(1, self.args.nsims + 1):
            if i not in existingIDs:
                np.savetxt(
                    os.path.join(MPath, "MessagesFile" + str(i).zfill(2) + ".csv"),
                    np.asarray([]),
                )
    ...
    def stats(self):
        # Get rows, cols
        DFForest = pd.read_csv(
            os.path.join(self.args.InFolder, "Forest.asc"),
            sep=" ",
            header=None,
            nrows=2,
        )
        ...
        if self.args.OutMessages:
            ...
            if self.args.spreadPlots or self.args.allPlots:
                ...
                for v in tqdm(range(totalPlots)):
                    StatsPrinter.GlobalFireSpreadEvo(
                        self._CoordCells, onlyGraph=True, version=v
                    )
                ...
                if self.args.HCells is not None:
                    HCarray = np.loadtxt(self.args.HCells, skiprows=1, delimiter=",")[
                        1:
                    ].astype(np.int)
            ...
            if self.args.plots or self.args.allPlots:
                ...
                for n in tqdm(range(1, self.args.nsims + 1)):
                    for v in range(1, 4):
                        StatsPrinter.SimFireSpreadEvoV2(
                            n,
                            self._CoordCells,
                            self._Colors,
                            H=None,
    ...
    def stats_Heur(self, OutFolder, HCells=None):
        ...
        if self.args.spreadPlots or self.args.allPlots:
            ...
            if HCells is not None:
                HCarray = np.loadtxt(HCells, skiprows=1, delimiter=",")[1:].astype(
                    np.int
                )
                self._GForestN = self._GForestN.flatten()
                for i in HCarray:
            ...
            StatsPrinter.ForestPlot(
                FBPlookup, self._GForestN, OutFolder, namePlot="InitialForest"
            )
    ...
    def heur(self, AvailCells=set(), BurntCells=set(), HarvestedCells=set()):
        ...
        if self.args.stats is False or (
            self.args.spreadPlots is False
            and self.args.plots is False
            and self.args.allPlots is False
        ):
            print("Reading data...")
            self.getData()
        ...
        # Heur object
        self._HeurObject = Heuristic(
            version=self.args.heuristic,  # Heuristic ID
        ...
        if not os.listdir(MessagePath) and self.env._heuristic >= 6:
            ...
            if self.args.valueFile is None:
                ...
                self._HeurObject.initGraph_FPV(
                    np.full(self._NCells, 1),  # Vol
                    ...
                    self.args.GPTree,
                )  # Global Propagation Tree option
            else:
                # User provides a custom value file with values for each cell
                customValue = np.loadtxt(
                    self.args.valueFile, delimiter=" ", dtype=np.float32
                )
                customValue = customValue.flatten()
                if len(customValue) != self._NCells:
                    print(
                        "[ERROR] Custom value matrix does not match the dimension of the forest, please check it"
                    )
                    print("[ERROR] Running with identical weights (1)")
                    customValue = np.full(self._NCells, 1)
                else:
                    print(
                        "Using custom value function ( from file",
                        self.args.valueFile,
                        ")",
                    )
                self._HeurObject.initGraph_FPV(
        ...
        if self.args.fdemand:
            DFractions = [
                ...
                0.003,
                0.004,
                0.005,  # Demand fractions (Finer version)
                0.01,
                0.015,
                0.02,
                0.025,
        ...
        DUnits = [
            int(dfr * len(self._AvailCells)) for dfr in DFractions[1:]
        ]  # Demand translated to units (based on available set)
        if self.args.valueFile is not None:
            self._CellUtility = np.loadtxt(
                self.args.valueFile, delimiter=" ", dtype=np.float32
            )
            self._CellUtility = self._CellUtility.flatten()
        else:
            self._CellUtility = np.full(
                shape=(self._NCells), fill_value=1
            )  # Cell's utility
        ...
        for fr in DFractions:
            ...
            if fr == -1:
                ...
                action, fitness = self._HeurObject.runHeur(
                    self._AvailCells.copy(),  # Available cells
                    ...
                    1,
                )  # Year (TODO: can be deleted later since we test all ddm.)
                actions = list(action)
                ...
                if self.args.noEvaluation is False:
                    ...
                    self.run_Heur(
                        os.path.join(csvPath, "Fraction" + str(fr)),
                        os.path.join(csvPath, SelHeur + "_" + str(fr) + ".csv"),
                    )
