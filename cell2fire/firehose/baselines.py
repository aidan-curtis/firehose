...
class NaiveAlgorithm(Algorithm):
    def __init__(self, env: FireEnv):
        ...
        self.flatten_idx_to_yx = {x + y * width: (y, x) for x in range(width) for y in range(height)}
    ...
    def predict(self, obs, **kwargs) -> Tuple[Any, Any]:
        ...
        if np.sum(cells_on_fire) <= 10:
            ...
            dist = [np.linalg.norm(np.array(yx) - np.array(self.ignition_point_yx)) for yx in fire_yx]

            chosen_fire_idx = -1
            while chosen_fire_idx in self.prev_actions:
                if not dist:
                    return 0, None
